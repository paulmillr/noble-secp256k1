...
const u8fr = (a) => Uint8Array.from(a);
const toU8 = (a, len) => u8fr(au8(a, len));
const M = (a, b = P) => {
    const r = a % b;
    return r >= _0 ? r : b + r;
};
...
class Point {
    ...
    // 0.4kb
    toHex(c = true) { return b2h(this.toBytes(c)); }
...
// @ts-ignore
const newsig = (r, s, recovery) => freeze({ r: agroup(r), s: agroup(s), recovery });
...
const verify = (sig, msgh, pub, opts = optV) => {
    ...
    let h, P; // secg.org/sec1-v2.pdf 4.1.4
    let r, s;
    // @ts-ignore
...
const challenge = (...args) => modN(b2n(taggedHash(T_CHALLENGE, ...args)));
const challengeAsync = async (...args) => modN(b2n(await taggedHashAsync(T_CHALLENGE, ...args)));
/**
 * Schnorr public key is just `x` coordinate of Point as per BIP340.
 */
const pubSchnorr = (privateKey) => {
...
const prepSigSchnorr = (message, privateKey, auxRand) => {
 ...
 */
const verifySchnorr = (s, m, p) => verifSchnorr(s, m, p, true);
const verifyAsyncSchnorr = async (s, m, p) => verifSchnorr(s, m, p, false);
const schnorr = {
