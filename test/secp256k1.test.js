...
const runTestsWithCrypto = (cn, c) => {
  if (globalThis.crypto !== c) {
    globalThis.crypto = c; // @ts-ignore
  }
  describe(`secp256k1 ${cn}`, () => {
    should('getPublicKey()', () => {
...
if (process.env['TEST_WEBCRYPTO'] === '1') {
  runTestsWithCrypto('webcrypto', webcrypto);
} else {
  runTestsWithCrypto('native', globalThis.crypto);
}
