...
describe('verify()', () => {
  ...
  describe('recoverPublicKey()', () => {
    ...
    it('recovers public key for recovery id 2/3 (R.x = r + n lift)', () => {
      ...
      const sig = new secp.Signature(2n, s).addRecoveryBit(3).toBytes('recovered');
      const pub = secp.recoverPublicKey(sig, msgHash, { prehash: false });
      eql(secp.verify(sig.slice(1), msgHash, pub, { prehash: false }), true);
      // Lift falling outside the base field must fail loudly: (n-1) + n > p.
      const invalid = new secp.Signature(CURVE_N - 1n, s).addRecoveryBit(2).toBytes('recovered');
      throws(() => secp.recoverPublicKey(invalid, msgHash, { prehash: false }));
    });
  });
});
...
describe('secp256k1 schnorr.sign()', () => {
  ...
  it('lift_x rejects x = 0 and x >= p', () => {
    ...
    throws(() => secp.__TEST.lift_x(0n));
    throws(() => secp.__TEST.lift_x(p));
    // x = p - 1: in-range but x^3 + 7 is a non-square, so no point exists
    throws(() => secp.__TEST.lift_x(p - 1n));
  });
