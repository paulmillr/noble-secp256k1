...
describe("secp256k1", () => {
  it("should verify just signed message", () => {
    fc.assert(fc.property(fc.hexa(), fc.bigInt(1n, secp256k1.PRIME_ORDER), (message, privateKey) => {
    ...
    fc.assert(
      fc.property(
        ...
        (message, wrongMessage, privateKey) => {
          ...
          expect(secp256k1.verify(signature, wrongMessage, publicKey)).toBe(message === wrongMessage);
        })
      );
  });
  it("should decode right encoded point with compresed hex", () => {
    fc.assert(fc.property(fc.bigUint(secp256k1.PRIME_ORDER), fc.integer(2, 3), (x, prefix) => {
  ...
  it("should decode right encoded signature with hex", () => {
    fc.assert(fc.property(fc.bigUint(secp256k1.PRIME_ORDER), fc.bigUint(secp256k1.PRIME_ORDER), (r, s) => {
