...
class SyncShaError extends Error {
...
class HmacDrbg {
  ...
  private hmacSync(...values: Uint8Array[]) {
    if (typeof utils.hmacSha256Sync !== 'function')
      throw new SyncShaError('utils.hmacSha256Sync is undefined, you need to set it');
    const res = utils.hmacSha256Sync!(this.k, ...values);
    if (res instanceof Promise)
      throw new SyncShaError('To use sync sign(), ensure utils.hmacSha256 is sync');
    return res;
  }
  incr() {
 ...
 * @param d private key
 * @param lowR whether signature should have its `r` lower than CURVE.n / 2. See sign().
 * @returns Signature with its point on curve Q OR undefined if params were invalid
 */
function kmdToSig(kBytes: Uint8Array, m: bigint, d: bigint, lowR = false): RecoveredSig | undefined {
...
type Ent = Hex | true;
type OptsOther = { canonical?: boolean; der?: boolean; extraEntropy?: Ent; lowR?: boolean }
 ...
 * low-r signatures are generated only when lowR flag is set to true.
...
async function sign(msgHash: Hex, privKey: PrivKey, opts: Opts = {}): Promise<SignOutput> {
  ...
  while (!(sig = kmdToSig(await drbg.generate(), m, d, opts.lowR))) await drbg.reseed();
...
function signSync(msgHash: Hex, privKey: PrivKey, opts: Opts = {}): SignOutput {
  ...
  // Step H3, repeat until k is in range [1, n-1]
  while (!(sig = kmdToSig(drbg.generateSync(), m, d, opts.lowR))) drbg.reseedSync();
  return finalizeSig(sig, opts);
}
...
function schnorrVerifySync(signature: Hex, message: Hex, publicKey: Hex): boolean {
  try {
    ...
    if (error instanceof SyncShaError) throw error;
    return false;
  }
}
...
export const utils = {
  ...
  taggedHashSync: (tag: string, ...messages: Uint8Array[]): Uint8Array => {
    if (typeof utils.sha256Sync !== 'function')
      throw new SyncShaError('utils.sha256Sync is undefined, you need to set it');
    let tagP = TAGGED_HASH_PREFIXES[tag];
    if (tagP === undefined) {
